...
{p 8 8 2}
{cmd:wmtmat} {it:name} [{it:{help using:using filename}}] [, {it:{help wmtmat##Options:options}}]
...
{synoptline}
{synopt :{it:name}}A matrix name required{p_end}
...
{synoptline}
{syntab:For all}
{synopt :{opth fmt:(format:fmt)}}Format all the values of the matrix{p_end}
{synopt :{opth rowsfmt:(format:fmtlist)}}Set the format of each row of the matrix separately{p_end}
{synopt :{opth colsfmt:(format:fmtlist)}}Set the format of each column of the matrix separately{p_end}
{synopt :{opth ti:tle(strings:string)}}Set the title for the table, {bf:Matrix matrix_name[rown,coln]} like {bf:Matrix B[6,5]} as the default{p_end}
